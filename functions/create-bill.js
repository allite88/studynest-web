...
exports.createBill = functions.https.onRequest(async (req, res) => {
  ...
  if (!uid || !email || !plan) {
    res.status(400).send('Missing uid / email / plan');
    return;
  }
  ...
  try {
    if (plan === 'books_order') {
      ...
      if (!items?.length || !shipping) {
        res.status(400).send('Missing items or shipping for books_order');
        return;
      }
    ...
    const params = new URLSearchParams({
      ...
      billCallbackUrl: `${siteUrl}/api/payment-notify`,
    ...
    let response;
    try {
      response = await fetch(`${apiBase}/index.php/api/createBill`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: params.toString(),
      });
    } catch (e) {
      res.status(502).json({ error: 'Cannot reach payment gateway' });
      return;
    }
